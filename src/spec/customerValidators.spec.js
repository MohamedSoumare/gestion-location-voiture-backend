...
describe('ClientValidators', () => {
  ...
  it('should pass when email is unique', async () => {
    ...
    await expect(
      ClientValidators.checkUniqueEmail('test@example.com')
  ...
  it('should pass when phone number is unique', async () => {
    ...
    await expect(
      ClientValidators.checkUniquePhoneNumber('1234567890')
    ).resolves.not.toThrow();
  });
  ...
  it('should pass when NNI is unique', async () => {
    ...
    await expect(
      ClientValidators.checkUniqueNni('9876543210')
    ).resolves.not.toThrow();
  });
  ...
  it('should throw an error if NNI already exists', async () => {
    ...
    await expect(ClientValidators.checkUniqueNni('9876543210')).rejects.toThrow(
      'NNI already in use'
    );
  });
  ...
  it('should pass when driving license is unique', async () => {
    ...
    await expect(
      ClientValidators.checkUniqueDrivingLicense('DL123456789')
    ).resolves.not.toThrow();
  });
  ...
  it('should throw an error if driving license already exists', async () => {
    ...
    await expect(
      ClientValidators.checkUniqueDrivingLicense('DL123456789')
    ).rejects.toThrow('Driving license already in use');
  });
});
